...
function initializeServiceWorker() {
  ...
  if ('serviceWorker' in navigator) {
    ...
    window.addEventListener('load', async () => {
      ...
      try {
        // B3. TODO - Register './sw.js' as a service worker (The MDN article "Using Service Workers" will help you here)
  ...
  return new Promise((resolve, reject) => {
    ...
    RECIPE_URLS.forEach(async (url) => {
      ...
      try {
        ...
        //            resolve() method.
        if (recpieArr.length === RECIPE_URLS.length) {
          saveRecipesToStorage(recpieArr);
          resolve(recpieArr);
        }
